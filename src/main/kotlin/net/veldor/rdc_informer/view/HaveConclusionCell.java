...
public class HaveConclusionCell<S, T> extends TableCell<S, T> {
    public static <S> Callback<TableColumn<S, String>, TableCell<S, String>> forTableColumn() {
        return forTableColumn(new DefaultStringConverter());
    }

    public static <S, T> Callback<TableColumn<S, T>, TableCell<S, T>> forTableColumn(final StringConverter<T> converter) {
        return list -> new HaveConclusionCell<>(converter);
    }
    ...
    protected void updateItem(final Cell<T> cell, final StringConverter<T> converter) {
        if (cell.isEmpty()) {
            ...
            Tooltip tooltip = new Tooltip(Objects.equals(text, "Нет") ? "Заключения ещё не загружены" : text);
     ...
     * </code>
     */
    public HaveConclusionCell() {
        this(null);
    }

    public HaveConclusionCell(StringConverter<T> converter) {
